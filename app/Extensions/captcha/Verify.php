...
class Verify {
    protected $config =	array(
        'seKey'     =>  'fdasfhfkdhfkjfhkfhdfdhfhg',   // 验证码加密密钥
    ...
    public function entry($id = '') {
        ...
        // 验证码使用随机字体
        $ttfPath = dirname(__FILE__) . '/Verify/' . (config('home.captchaType')==2 ? 'zhttfs' : 'ttfs') . '/';
        ...
        if(config('home.captchaType')==2){ // 中文验证码
        ...
        }else if(config('home.captchaType')==0){
            $firstNum = rand(1, 9);
            $secondNum = rand(1, 9);
            $operator = $firstNum > $secondNum ? '-' : '+';
            $mathArr = $firstNum . $operator . $secondNum . '=';
            $code[] = ($firstNum > $secondNum) ? ($firstNum-$secondNum) : ($firstNum+$secondNum);
            imagettftext($this->_image, $this->fontSize, 0, 38, $this->fontSize*1.6, $this->_color, $this->fontttf, $mathArr);
        }else{
            for ($i = 0; $i<$this->length; $i++) {
