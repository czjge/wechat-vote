...
namespace App\Http\Controllers\Home\v35;
...
use App\Repositories\vote\v35\VoteRepository as voteRep;
...
class VoteController extends HomeBaseController
{
    ...
    private $indexListCacheKey = 'scmy.vote.index.list.35';
    private $rankListCacheKey = 'scmy.vote.rank.list.35';
    protected $voteRep;
...
//        }
    ...
    public function getDoRegister (Request $request) {
        ...
        $father_info = explode(',', $data['father']);
        $data['name'] = $father_info[0] . '家庭';

        $main_data = array_slice($data, 5);

        DB::beginTransaction();
        if ($candidate_id = DB::table('candidate')->insertGetId($main_data)) {
            $extra_data = [
                'candidate_id' => $candidate_id,
                'values'       => serialize([
                    'group'       => $data['group'],
                    'address'     => $data['address'],
                    'father_info' => $data['father'],
                    'mother_info' => $data['mother'],
                    'kid_info'    => $data['kid'],
                ]),
            ];
