...
namespace App\Http\Controllers\Home\v5;
...
use App\Repositories\vote\v5\VoteRepository as voteRep;
...
class VoteController extends HomeBaseController
{
    ...
    private $indexListCacheKey = 'scmy.vote.index.list.5';
    private $rankListCacheKey = 'scmy.vote.rank.list.5';
    ...
    public function getIndex (Request $request) {
        ...
        $type = $request->has('type') ? (int) $request->input('type') : 0;
...
//                        });
        ...
        if ($keywords) {
            ...
            $builder = $builder->where(function ($query) use ($keywords){
                ...
                $query->orWhere('of_hos', 'like', '%'.$keywords.'%');
            })->where('type', '<>', 2);
        } else {
        ...
        if ($type!=2) {
            if ($vote->index_sort_type == 1) {
                $builder = $builder->orderBy('no', 'desc'); // 按编号降序
            } elseif ($vote->index_sort_type == 2) {
                $builder = $builder->orderBy('no', 'asc'); // 按编号升序
            } elseif ($vote->index_sort_type == 3) {
                $builder = $builder->orderBy('num', 'desc')->orderBy('no', 'asc'); // 按票数降序
            } elseif ($vote->index_sort_type == 4) {
                $builder = $builder->orderBy(DB::raw('convert(name using gbk)')); // 按选手姓名的拼音排序
            }
        }
...
//        }
        // 获取必要的扩展字段信息
        foreach ($candidates as $key => $candidate) {
            $extra_info = FieldValue::where('candidate_id', '=', $candidate->id)->first();
            $extra_info = unserialize($extra_info['values']);
            $candidates[$key]->title = $extra_info['title'];
        }
        ...
        // 总参赛选手数
        $candidates_num = DB::table('candidate')->where('vote_id', '=', $id)->whereIn('status', [0, 2])->whereIn('type', [0,1])->count();
...
//        }
        ...
        // 获取必要的扩展字段信息
        $extra_info = FieldValue::where('candidate_id', '=', $cid)->first();
        $extra_info = unserialize($extra_info['values']);
        $candidate->title = $extra_info['title'];
        $candidate->of_dep = $extra_info['of_dep'];
        $candidate->of_hos = $extra_info['of_hos'];
        $candidate->goodat = $extra_info['goodat'];
        $candidate->schedule_time = $extra_info['schedule_time'];
...
//        }
        ...
        if ($show) {
            $view_path = 'app.vote.'.$id.'.top10-info';
        } else {
            $view_path = 'app.vote.'.$id.'.info';
        }
    ...
    public function getRank (Request $request) {
        ...
        if (env('USE_REDIS_CACHE')) {
            ...
            if (in_array($type, [2, 3])) {
                $view_path = 'app.vote.'.$id.'.top10';
                if ($type==3) {
                    $rank_list = DB::table('candidate')->where('vote_id', '=', $id)->whereIn('name', [
                        '魏智林', '陈立翠', '朱蔓佳', '陈月', '陈犀玮',
                        '罗芯怡', '李秀亮', '孟晓露', '刘小凡', '陈怡'
                ...
                $view_path = 'app.vote.'.$id.'.rank';
                $rank_list = DB::table('candidate')->where('vote_id', '=', $id)->whereIn('status', [0,2])->where('type', '=', $type)->orderBy('num', 'desc')->take($vote->rank_num)->get();
            }
        }

        foreach ($rank_list as $k => $v) {
            $rank_list[$k]->url = route('home.vote.getInfo'.$v->vote_id, ['cid'=>$v->id]) . '?id=' . $id;
            $extra_info = FieldValue::where('candidate_id', '=', $v->id)->first();
            $extra_info = unserialize($extra_info['values']);
            $rank_list[$k]->title = $extra_info['title'];
        }
