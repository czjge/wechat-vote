...
class VoteController extends HomeBaseController
{
    ...
    public function getIndex (Request $request) {
        ...
        if($type==1){
            ...
            // 选手列表
            if ($keywords) {
                $builder = DB::table('candidate')->where('vote_id', '=', 3)->whereIn('status', [0, 2])->where('name', 'like', '%'.$keywords.'%');
...
//                        });
                ...
                $builder = DB::table('candidate')->where('vote_id', '=', 3)->where('type', '=', 0)->whereIn('status', [0, 2]);
            }
