...
namespace App\Http\Controllers\Home\v26;
...
use App\Repositories\vote\v26\VoteRepository as voteRep;
...
class VoteController extends HomeBaseController
{
    ...
    private $indexListCacheKey = 'scmy.vote.index.list.26';
    private $rankListCacheKey = 'scmy.vote.rank.list.26';
    protected $voteRep;
...
//        }
    ...
    public function postDoSthByType(Request $request)
    {
        ...
        if ($type == 1) {
            ...
            // 一个微信号只能预约一次
            if (DB::table('registration_log')->where('user_id', '=', $wid)->where('vote_id', '=', $vote_id)->first()) {
                ...
                if ($registration_left_num > 0) {
                    ...
                    if (DB::table('registration')->insert($data) && DB::table('registration_log')->insert(['user_id'=>$wid, 'vote_id'=>$vote_id])) {
                        ...
                        if ($re !== false) {
                            ...
                            $text = '';
                            if ($time == 'sw' && $no <= 10) {
                                $text = '【成都商报四川名医】通知:"健康儿童 幸福蓉城"义诊活动,恭喜您已成功预约' . $candidate->name . '医生,请凭此短信于8月19日（星期天）上午9点前至日月大道1617号成都市妇女儿童中心医院儿科大楼一楼外右侧取号再免费义诊.特别提醒:超过9:30未取号视为放弃;请遵守现场医疗秩序,按现场领取序号就医.';
                            } elseif ($time == 'sw' && $no <= 20) {
                                $text = '【成都商报四川名医】通知:"健康儿童 幸福蓉城"义诊活动,恭喜您已成功预约' . $candidate->name . '医生,请凭此短信于8月19日（星期天）上午10点前至日月大道1617号成都市妇女儿童中心医院儿科大楼一楼外右侧取号再免费义诊.特别提醒:超过10:30未取号视为放弃;请遵守现场医疗秩序,按现场领取序号就医.';
                            } else if ($time == 'sw' && $no <= 30) {
                                $text = '【成都商报四川名医】通知:"健康儿童 幸福蓉城"义诊活动,恭喜您已成功预约' . $candidate->name . '医生,请凭此短信于8月19日（星期天）上午11点前至日月大道1617号成都市妇女儿童中心医院儿科大楼一楼外右侧取号再免费义诊.特别提醒:超过11:15未取号视为放弃;请遵守现场医疗秩序,按现场领取序号就医.';
                            } else {
                                $text = '【成都商报四川名医】通知:"健康儿童 幸福蓉城"义诊活动,恭喜您已成功预约' . $candidate->name . '医生,请凭此短信于8月19日（星期天）下午1点前至日月大道1617号成都市妇女儿童中心医院儿科大楼一楼外右侧取号再免费义诊.特别提醒:超过1:50未取号视为放弃;请遵守现场医疗秩序,按现场领取序号就医.';
                            }
                            $sms_post_info = [
