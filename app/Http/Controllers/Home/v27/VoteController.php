...
namespace App\Http\Controllers\Home\v27;
...
use App\Repositories\vote\v27\VoteRepository as voteRep;
...
class VoteController extends HomeBaseController
{
    ...
    private $indexListCacheKey = 'scmy.vote.index.list.27';
    private $rankListCacheKey = 'scmy.vote.rank.list.27';
    protected $voteRep;
...
//        }
    ...
    public function getRank (Request $request) {
        ...
        // 科室列表
        $dep_list_raw = [
            '呼吸内科','消化内科','心血管内科','肾内科','内分泌科','风湿免疫科','胃肠外科','甲状腺乳腺外科','肝胆胰外科','血管外科','营养科','骨科',
            '泌尿外科','胸心外科','神经外科','妇科','产科','儿科','感染传染科','肿瘤科',
            '耳鼻喉科','神经内科','疼痛科','康复科','血液内科','皮肤性病科',
        ];
        ...
        // 精神心理科-精神 心理 抑郁
        $dep_list_js_num = DB::table('candidate')
            ->where('candidate.vote_id', '=', $id)
            ->whereIn('candidate.status', [0, 2])
        ...
        $xxx = new \stdClass();
        $xxx->dep_name = '精神心理科';
        $xxx->dep_total_num = $dep_list_js_num . '';
        $insert_key = '';
        foreach ($dep_list as $key => $dep_list_item) {
            if ($dep_list_js_num >= (int) $dep_list_item->dep_total_num) {
                $insert_key = $key;
                break;
            }
        }
        for ($i=count($dep_list) ; $i>=$insert_key ; $i--) {
            if ($i == $insert_key) {
                $dep_list[$i] = $xxx;
            } else {
        ...
        // 感染传染科-感染 传染
        $dep_list_gr_num = DB::table('candidate')
            ->where('candidate.vote_id', '=', $id)
            ->whereIn('candidate.status', [0, 2])
        ...
        $xxx = new \stdClass();
        $xxx->dep_name = '感染传染科';
        $xxx->dep_total_num = $dep_list_gr_num . '';
        $insert_key = '';
        foreach ($dep_list as $key => $dep_list_item) {
            if ($dep_list_gr_num >= (int) $dep_list_item->dep_total_num) {
                $insert_key = $key;
                break;
            }
        }
        ...
        // 四川大学华西医院的“美容整形-烧伤外科”，成都市第三人民医院下面“美容科”、和其他医院：烧伤整形外科 、
        // 整形烧伤科 、烧伤整形科的医生，整合到“整形外科”榜单下。
        $dep_list_gr_num = DB::table('candidate')
            ->where('candidate.vote_id', '=', $id)
            ->whereIn('candidate.status', [0, 2])
            ->where(function ($q) {
        ...
        // 成都市第二人民医院：医疗美容科、
        //成都中医药大学附属医院：医学美容科  的意思，整合到“美容皮肤科”榜单下
        $dep_list_gr_num = DB::table('candidate')
            ->where('candidate.vote_id', '=', $id)
            ->whereIn('candidate.status', [0, 2])
            ->where(function ($q) {
