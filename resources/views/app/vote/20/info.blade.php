...
@section('content')
    ...
    <div class="info-content">
        <div class="info-item">
            姓名或昵称：<span>{{ $candidate->name }}</span>
            <div class="votes-btn" data-url="{{ $candidate->ajax_vote_url }}"><i class="icon-flower"></i> 给Ta送花</div>
        </div>
        <div class="info-item">
            兴趣爱好：<span>{{ $candidate->hobby }}</span>
        </div>
        <div class="info-item">
           帮帮团评价：
            <div class="info-judge">{{ $candidate->desc }}</div>
        </div>
        <div class="info-item">
            照片：
            <div class="info-img">
                <img src="{{ $qiniu_cdn_path }}/images/vote/{{ Input::get('id') }}/info/{{ $candidate->id }}.png" width="100%"/>
            </div>
        </div>
    </div>
