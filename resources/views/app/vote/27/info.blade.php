...
@section('selfcss')
    ...
    <link href="{{ $qiniu_cdn_path }}/css/vote/{{ $vote->id }}/tn_code.css" rel="stylesheet" type="text/css">
    <style>
        .footer_main{opacity: 1}
    </style>
    ...
    <style>
        ...
        .tncode{font-size: 14px}
    </style>
@endsection

@section('content')
    <div class="log-head"></div>
    <div class="info-img"><img src="@if(file_exists(public_path().'/images/vote/'.$vote->id.'/info/'.$candidate->name.'.png')) {{ $qiniu_cdn_path }}/images/vote/{{ $vote->id }}/info/{{ $candidate->name }}.png @elseif(''==$candidate->pic_url) {{ $qiniu_cdn_path }}/images/default_doc_avatar.jpg @elseif(false !== strpos($candidate->pic_url, 'https')) {{ $candidate->pic_url }} @else {{ $qiniu_cdn_path }}/storage/{{ $candidate->pic_url }} @endif"></div>
    <div class="info-detail ellipsis"><span>{{ $candidate->name }}</span>{{ $candidate->tit }}</div>
    <div class="info-hos ellipsis">{{ $candidate->hos }}&nbsp;{{ $candidate->dep }}</div>
    <div class="info-logo"></div>
    <div class="info-text">{!! $candidate->desc !!}</div>

    <div class="tncode" style="text-align: center;margin: 100px auto;display: none;"></div>

    <div class="info-btn">给TA投票</div>
...
@section('selfjs')
    ...
    <script src="{{ $qiniu_cdn_path }}/js/vote/{{ $vote->id }}/tn_code.js" type="text/javascript"></script>
    <script type="application/javascript">
        $(function () {
            ...
            var captchaIsOn = 0;
            $('#captcha_code_btn, .info-btn').click(function () {
                ...
                $.ajax({
                    ...
                    dataType : "json",
                    data: {type: 3, vid: '{{ Input::get('id') }}', cid: '{{ $candidate->id }}'},
                    beforeSend: function () {
                        loading('请稍后');
                        submintVoteStatus = 1;
                    },
                    ...
                    success: function(data){
                        ...
                        if (captchaIsOn == 1 && captchaShowStatus == 0) {
                            $('.tncode').css('display', 'block');
            ...
            function afterCheckCaptcha () {
                ...
                if (captchaIsOn == 1) {
                    $TN.refresh();
                    $TN._tncode.innerHTML = '点击按钮进行验证';
                    $('.tncode').css('display', 'none');
