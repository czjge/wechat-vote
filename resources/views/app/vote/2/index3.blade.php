            ...
            <ul class="list-block specialist-list">
                ...
                @foreach ($retval as $item)
                    <li class="specialist-item clear">
                        <a href="{{ route('home.vote.getInfo'.$id, ['cid'=>$item->id]).'?id='.$id.'&type=3' }}" class="clear">
                            <div class="left-sec-con wid-auto">
                                <div class="rank-img wid-small"><img src="<?php if($item->pic_url==''){echo $qiniu_cdn_path.'/images/default_doc_avatar.jpg';}else{echo 'http://qiniu.langzoo.com'.$item->pic_url;}?>" ></div>
                            </div>
                            <div class="left-sec-con wid-auto mrg-t20">
                                <span class="rank-name">{{ $item->name }}</span><span class="rank-name rank-name-ks font-s26">{{ $item->title }}</span>
                            </div>
                            <div class="right-sec-con mrg-t24">
                                <span class="rank-name rank-name-yy font-s26">{{ $item->of_hospital }}</span>
                            </div>
                        </a>
                    </li>
                @endforeach
