    ...
    <div class="row">
        <div class="col-md-12">
            <div class="box box-info">
                ...
                <form role="form" action="{{ route('admin.doctor.postEdit') }}" method="post" pjax-container onsubmit="return checkDocForm()">
                    {!! csrf_field() !!}
                    ...
                    <div class="box-body">
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2-hospital" name="hid" id="hid">
                                @foreach ($hospitalList as $item)
                                    <option value="{{ $item->id }}" @if ($doctor->hid == $item->id)selected="selected"@endif >{{ $item->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2-department" name="department_id" id="department_id">
                                @foreach ($initialDeps as $item)
                                    <option value="{{ $item->id }}" @if ($doctor->department_id == $item->id)selected="selected"@endif >{{ $item->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" name="prof" id="prof" value="{{ $doctor->prof }}" placeholder="" maxlength="255">
                        </div>

                        <div class="form-group">
                            <label for="avatar" class="col-sm-2 control-label">头像</label>

                            <input type="file" class="form-control fileupload" value="{{ $doctor->avatar }}">
                            <span class="help-block">请上传170px X 170px分辨率图片，支持jpg和png格式</span>
                            <div class="img-div" data-name="avatar">
                                <input type='hidden' name='avatar' value='{{ $doctor->avatar }}'/>
                            </div>
                        </div>

                        <div class="form-group">
                            <label for="phone" class="col-sm-2 control-label">手机号码</label>
                            <input type="text" class="form-control" name="phone" id="phone" value="{{ $user->phone or '' }}" placeholder="" maxlength="32">
                        </div>

                        <div class="form-group">
                            <label for="shanchang" class="col-sm-2 control-label">擅长</label>
                            <textarea class="form-control" name="shanchang" id="shanchang" rows="10">{{ $doctor->shanchang }}</textarea>
                        </div>

                        <div class="form-group">
                            <label for="intro" class="col-sm-2 control-label">个人简介</label>
                            <textarea class="form-control" name="intro" id="intro" rows="10">{{ $doctor->intro }}</textarea>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" multiple="multiple" name="circle_ids[]" id="circle_ids" data-placeholder="收数据的请勿选择">
                                <?php foreach($circles as $vo){?>
                                    <option value="<?php echo $vo['id'];?>" <?php if(in_array($vo['id'],$_circle_ids)){echo "selected='selected'";}?> ><?php echo $vo['name'];?></option>
                                <?php }?>
                            </select>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control" name="label" id="label">
                                ...
                                @foreach ($labels as $item)
                                    <option value="{{ $item->id }}" @if ($doctor->label == $item->id)selected="selected"@endif >{{ $item->disease_label }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control" name="status" id="status">
                                ...
                                <option value="2" @if ($doctor->status == '2')selected="selected"@endif >审核未通过</option>
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control" name="is_consultable" id="is_consultable">
                                <option value="0" @if ($doctor->is_consultable == '0')selected="selected"@endif >否</option>
                                <option value="1" @if ($doctor->is_consultable == '1')selected="selected"@endif >是</option>
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" name="consult_price" id="consult_price" value="{{ $doctor->consult_price }}" placeholder="">
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control" name="schedule_text" id="schedule_text" rows="10">{{ $doctor->schedule_text }}</textarea>
                        </div>
