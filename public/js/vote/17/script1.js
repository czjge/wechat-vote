    ...
    jQuery(function() {
        ...
        uploader.on( 'uploadSuccess', function( file, response) {
            if (response._raw != undefined) {
                ...
                loading(false);
                $('#reg-form').find('[name=pic_url]').val(dataarr[1]);
            } else {
                if(response == 3){
                    loading(false);
                    tusi('上传图片过大');
        ...
        */
        ...
        function addFile( file ) {
            var $li = $( '<li id="' + file.id + '">' +
                ...
                '</li>' ),
                $btns = $('<div class="file-panel"><span><i class="icon-delete"></i> </span></div>').appendTo( $li ),
